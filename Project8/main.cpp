...
int main()
{
	...
	srcMat = imread("D:\\1.png", 0);
	dstMat = imread("D:\\1.png");

	Mat src1Mat = 255 - srcMat;
	threshold(src1Mat, binMat, 150, 255, THRESH_OTSU);

	Mat element = getStructuringElement(MORPH_ELLIPSE, Size(7, 8));
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		for (int j = 0; j < 4; ++j) {
			...
			kuanchangbi[i] = Y / X;
			float mianji = X * Y;
			if (kuanchangbi[i] > 0.8&&kuanchangbi[i] < 1.1&&mianji>1000)
			{
